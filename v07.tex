...
\fancyhead[R]{\calligra\Large 04.11.2013}
}
%Kopfzeile links bzw. innen
\fancyhead[L]{\calligra\Large Vorlesung Nr. 7}
%Kopfzeile rechts bzw. außen
...
\subsection*{Beweise}
\enum{
	\item $|a_n + b_n - (a + b)| ≤ |a_n - a| + |b_n - b| =: S_n$\\
	Zu $\e > 0$ wähle $n_0\in \N$ so groß, dass $|a_n - a|<\frac{\e}{2}$, $|b_n - b|< \frac{\e}{2} \forall n ≥ n_0$
	\item Übung
	\item Wegen (2) ohne Einschränkung $a_n = a = 1$\\
	$|b_n|>0$ und $b_n \to b \Rarr \exists N \in \N: |b_n - b|<\frac{|b|}{2} \forall n ≥ N$\\
	$n≥N \Rarr |b_n| = |b-(b-b_n)| ≥ |b| - |b-b_n| > \frac{|b|}{2}$\\
	Ferner: $\vert \frac{1}{b_n} - \frac{1}{b} \vert = 
	\frac{\vert b - b_n \vert}{\vert b_n \vert \cdot |b|} = \frac{1}{|b|} \cdot \frac{2}{|b|} \cdot |b - b_n| (\forall n ≥ N)$\\
	Sei $\e > 0$. $b_n \to b \Rarr \exists n_0 ≥ N: |b - b_n| < \frac{|b|^2}{2} \cdot \e \forall n ≥ n_0$\\
	$\Rarr \left\vert \frac{1}{b_n} - \frac{1}{b}\right\vert < \e \forall n ≥ n_0$
	\item Sei $|a_n - a| < \e \forall n ≥ n_o$\\
	$\Rarr ||a_n| - |a|| ≤ |a_n - a| < \e \forall n≥n_0$
}
