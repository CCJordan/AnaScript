...
$f$ sei differenzierbar in $x_0 \Rarr f$ ist stetig in $x_0$.\\

\subsection*{Beweis}
Sei $x \neq x_0$.\\
$\Rarr f(x)-f(x_0) = \underbrace{\frac{f(x)-f(x_0)}{x-x_0}}_{\to f'(x_0)} \cdot \underbrace{(x-x_0)}_{\to 0} \to 0$ für $x \to x_0 \Rarr$ Behauptung \qed

\section{Satz: Ableitungsregeln}
Seien $f,g$ differenzierbar in $x_0 \in D$\\
$\Rarr f+g, f \cdot g$ sind differenzierbar in $x_0$ und im Fall $g(x) \neq 0$ auch $\frac{f}{g}$.\\
Dabei gelten:
\enum{
\item $(f+g)'(x) = f'(x)+g'(x)$\\
$c \in \R \Rarr (c \cdot f)'(x) = c \cdot f'(x)$
...
$\underset{\text{\ref{11.2} + Regeln}}{\Rarr} p$ differenzierbar auf $\R$, $p'(x) = n \cdot a_n x^{n-1} + \ldots + a_1$
\item $R(x) = \frac{p(x)}{q(x)} \Rarr R$ differenzierbar auf $\{x \in \R: q(x) \neq 0\}$\\
Beispiel: $R(x) = \frac{1}{x^k} (k \in \N)$\\
$\Rarr \forall x \in \R \setminus \{0\}$ ist $R'(x) \underset{\text{Q-R}}{=} - \frac{n \cdot x^{n-1}}{(x^n)^2} = -\frac{n}{x^{n+1}}$\\
...
$\Rarr \left(\frac{f}{0}\right)' \underset{\text{Produktr.}}{=} \frac{f'}{g} + f \cdot \left(\frac{-g'}{g^2}\right) = \frac{f'g-fg'}{g^2}$
}

\section{Satz: Kettenregel}
Seien $f: D \to E$ und $g: E \to \R$ Funktionen.\\
$f$ sei differenzierbar in $x_0 \in D$, $g$ sei differenzierbar in $f(x_0)$\\
$\Rarr g \circ f$ ist differenzierbar in $x_0$ mit \fbox{$(g \circ f)'(x_0) = g'(f(x_0)) \cdot \underbrace{f'(x_0)}_{\text{Nachdiff. von } f}$}
...
$\overset{ln'(x)=\frac{1}{x}}{\Rarr} p_s'(x) = s \cdot e^{s \cdot ln(x)} \cdot \frac{1}{x} = s \cdot x^{s-1}$
\item $f$ differenzierbar in $x \in \R \Rarr (e^{f(x)})' = e^{f(x)} \cdot f'(x)$
\item $f>0 \Rarr (ln(f))' = \frac{f'}{f}$ ``logarithmische Ableitung von $f$''
}
